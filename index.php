<?php
include ("components/header.php");
?>

<div class="container">
    <div class="row">
        <div class="col-md-12 text-center top-abstand">
            <h1>Projekt WebFit Markup</h1>
            <h4>Willkommen beim Markup :)</h4>
        </div>
        <div class="col-md-12 text-center top-abstand">
           <?php include ("components/how-to.php") ?>
        </div>
    </div>
</div>

<hr >
<div class="container">
    <div class="row">
        <div class="col-md-6">
            <h3>Komponenten</h3>
            <ul>
                <li><a href="index.php">How-To-Component</a></li>
                <li><a href="button-examples.php">Buttons</a></li>
                <li><a href="animation-examples.php">Animations</a></li>
                <li><a href="form-examples.php">Form</a></li>
                <li><a href="alert-examples.php">Alerts</a></li>
                <li><a href="addButton-examples.php">Add Button</a></li>
                <li><a href="home_example.php">Home</a></li>
            </ul>
        </div>
        <div class="col-md-6">
            <h3>Beispiel-Seiten</h3>
                <li><a href="index.php">Demo-Page</a></li>
                <li><a href="https://daneden.github.io/animate.css/" target="_blank">animate.css | Animationen</a></li>
                <li><a href="https://www.youtube.com/watch?v=S2KCXKAView&t=434s" target="_blank">animate.css | Tutorial (ab Minute 7)</a></li>
        </div>
    </div>
</div>

<?php
include ("components/footer.php");
?>
